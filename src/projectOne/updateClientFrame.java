...
public class updateClientFrame extends JFrame {
	...
	public updateClientFrame(final String param) {
		...
		JLabel lblAddAgent = new JLabel("Update Client");
		...
		btnAddAgent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					String query = "update Client set FirstName = ?,SurName = ?,UserName = ?,EmailAddress = ?,ContactPerson = ?,PhoneNumber = ? where UserName = ?";
					...
					if(tf_fname.getText().isEmpty()||tf_surname.getText().isEmpty()||tf_username.getText().isEmpty()||tf_email.getText().isEmpty()||tf_contactPerson.getText().isEmpty()){
						...
						pst.execute();
						JOptionPane.showMessageDialog(null, "Client updated successfuly", "Note!", JOptionPane.INFORMATION_MESSAGE);
					}
					pst.close();
				} catch (Exception e) {
